...
# ---------------------------------------------------------------------------
# Outside papers computation — must run before sidebar to show count there
# ---------------------------------------------------------------------------
if (base_graph is not None
    ...
    _sel_idx_pre     = {v.index for v in base_graph.vs if v["name"] in _sel_names_pre}
    _link_count_pre: dict[str, int] = defaultdict(int)
    for _idx in _sel_idx_pre:
        for _nb in base_graph.neighbors(_idx):
            _nb_name = base_graph.vs[_nb]["name"]
            if _nb_name not in _sel_names_pre:
                _link_count_pre[_nb_name] += 1
    _outside_pre = {n for n, c in _link_count_pre.items() if c >= _threshold_pre}
    st.session_state["outside_papers"]       = _outside_pre
    st.session_state["outside_papers_count"] = len(_outside_pre)
else:
    st.session_state["outside_papers_count"] = None

# ---------------------------------------------------------------------------
# Sidebar — always rendered regardless of dataset state
# ---------------------------------------------------------------------------
with st.sidebar:
    with st.expander("Saved collections", expanded=False):
        if base_graph is None:
            ...
            with st.expander(_caption, expanded=False):
    ...
    with st.expander("Run clustering", expanded=False):
        st.caption("Clustering runs on the active collection selected in Saved collections.")
        run = st.button(
            _run_label,
            type="secondary",
            use_container_width=True,
    ...
    st.markdown("### Interact with the map")
    ...
    with st.expander("Cluster colors", expanded=False):
        if not _has_results:
            ...
            if st.button("Default (no meaning)", key="clear_colors", use_container_width=True):
                st.session_state.pop("color_mode", None)
                st.session_state.pop("color_values", None)
                st.session_state.pop("color_label", None)
                st.session_state.pop("color_raw_values", None)
                st.session_state.pop("color_max_val", None)

            _ci1, _ci2 = st.columns([4, 1])
            with _ci1:
                st.text_input("Frequency of this term (one word)", key="search_term", placeholder="e.g. network",
            ...
            _co1, _co2 = st.columns([4, 1])
            with _co1:
                st.text_input("Number of edges from this cluster", key="conn_input", placeholder="e.g. 3",
    ...
    with st.expander("Select documents", expanded=False):
        if not _has_results:
            ...
            st.text_input("sel_input", key="sel_input", placeholder="e.g. 0 3 7",
                label_visibility="collapsed",
                help="Space-separated cluster IDs. Use Add to select, Remove to deselect.")
            _sa1, _sa2 = st.columns(2)
            with _sa1:
                st.button("Add cluster", key="add_sel", use_container_width=True, on_click=_cb_add_selection)
            with _sa2:
                st.button("Add all clusters", key="sel_all", use_container_width=True, on_click=_cb_select_all,
                    help="Select all clusters currently in the map.")
            _sa3, _sa4 = st.columns(2)
            with _sa3:
                st.button("Remove cluster", key="rem_sel", use_container_width=True, on_click=_cb_remove_selection)
            with _sa4:
                st.button("Remove all clusters", key="clear_sel", use_container_width=True, on_click=_cb_clear_selection)

            if st.session_state.get("_sel_warning"):
                st.warning(st.session_state["_sel_warning"])

            _sel_sidebar_clusters = st.session_state.get("selected_clusters", set())
            ...
            st.checkbox(
                ...
                disabled=not _sel_sidebar_clusters,
                on_change=lambda: st.session_state.update(
                    expand_outside_enabled=st.session_state.get("expand_outside_cb", False)
                ),
            )
            if st.session_state.get("expand_outside_enabled") and _sel_sidebar_clusters:
                st.number_input(
                ...
                _outside_count = st.session_state.get("outside_papers_count")
                if _outside_count is not None:
                    st.caption(f"{_outside_count:,} connected documents found.")

            if _sel_sidebar_clusters:
                _cg_sel = st.session_state["cluster_result"]["cg"]
                _total_sel = sum(v["size"] for v in _cg_sel.vs if v["community_id"] in _sel_sidebar_clusters)
                st.caption(f"Selected: {', '.join(str(i) for i in sorted(_sel_sidebar_clusters))} — {_total_sel:,} documents")
                _sn1, _sn2 = st.columns([3, 2])
                with _sn1:
                    st.text_input(
                        "Collection name",
                        key="_save_sel_name",
...
try:
    ...
    conn_input  = st.session_state.get("conn_input", "")
    selected_clusters = st.session_state.get("selected_clusters", set())

    outside_papers: set[str] = st.session_state.get("outside_papers", set())
    ...
    if layout == "Bubble chart":
        ...
        if st.session_state.get("_bubble_layout_key") != _layout_key:
            ...
            with st.spinner("Computing bubble layout…"):
                _placed = build_graph(build_node_list(cg), max_starts=max_starts)
            st.session_state["_bubble_placed"]     = _placed
            st.session_state["_bubble_layout_key"] = _layout_key
        fig = build_bubble_figure(cg, labels, show_edges=show_edges, max_starts=max_starts, term_freq=color_values, color_label=color_label, hover_label=hover_label, raw_values=color_raw_values, selected=selected_clusters, top_docs=top_docs_n, top_nouns=top_nouns_n, placed=st.session_state["_bubble_placed"])
